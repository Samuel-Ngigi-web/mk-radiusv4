...
Radius._load_dictionary = function (content) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      attributes_map[attr_vendor][match[2]] =
        attributes_map[attr_vendor][match[3]];
    ...
    if (match) {
      ...
      attributes_map[attr_vendor][match[2]][ATTR_REVERSE_ENUM][match[3]] =
        match[4];
...
Radius.decode = function (args) {
  ...
  if (
    ...
    //No.1 Line 333: changed "new Buffer" to "Buffer.alloc()"
    ...
    if (checksum.toString() != this.authenticator.toString()) {
      console.log(
        "Checksome-to-string: " +
          checksum.toString() +
          " didn't match authenticator-to-string: " +
...
Radius.zero_out_message_authenticator = function (attributes) {
  ...
  for (var i = 0; i < new_attrs.length; i++) {
    ...
    if (attr[0] == ma_id) {
      //No.2 Line 359: changed "new Buffer" to "Buffer.alloc()"
...
Radius._verify_request_message_authenticator = function (args, request) {
  ...
  var expected_ma = this.calculate_message_authenticator(
    reencoded,
    args.secret
  );
  ...
  if (orig_ma.toString() != expected_ma.toString()) {
    this.error(
      new Radius.InvalidSecretError(
        "decode: Message-Authenticator mismatch (possible shared secret mismatch)",
        request
      )
    );
  }
...
Radius.verify_response = function (args) {
  ...
  if (
    !args ||
    !Buffer.isBuffer(args.request) ||
    !Buffer.isBuffer(args.response)
  ) {
    this.error(
      "verify_response: must provide raw request and response packets"
    );
    return;
  }
  ...
  //No.3 Line 443: changed "new Buffer" to "Buffer.alloc()"
  ...
  if (
    ...
    args.response.slice(AUTH_START, AUTH_END).toString()
  ) {
    return false;
  }
...
Radius._verify_response_message_authenticator = function (args) {
  ...
  if (parsed_request.attributes["Message-Authenticator"]) {
    ...
    var expected_response = this.encode({
      ...
      attributes: this.zero_out_message_authenticator(
        parsed_response.raw_attributes
      ),
    });
  ...
  while (data.length > 0) {
    ...
    if (attr_info[ATTR_MODIFIERS]["encrypt=1"]) {
      ...
      switch (attr_info[ATTR_TYPE]) {
        ...
        case "integer":
          if (attr_info[ATTR_MODIFIERS]["has_tag"]) {
            //No.4 Line 554: changed "new Buffer" to "Buffer.from()"
...
Radius.encrypt_field = function (field) {
  ...
  //No.5 Line 624: changed "new Buffer" to "Buffer.alloc()"
...
Radius._crypt_field = function (field, is_decrypt) {
  // No.6 Line 637: changed "new Buffer" to "Buffer.alloc()"
  ...
  for (var i = 0; i < field.length; i = i + 16) {
    ...
    //No.7 Line 649: changed "new Buffer" to "Buffer.from()"
    var hash = Buffer.from(hasher.digest("binary"), "binary");
    //No.8 Line 651: changed "new Buffer" to "Buffer.alloc()"
...
Radius.encode_response = function (args) {
  ...
  var response = this.encode({
    ...
    add_message_authenticator:
      packet.attributes["Message-Authenticator"] != null,
  });
...
Radius.encode = function (args) {
  ...
  //No.9 Line 713: changed "new Buffer" to "Buffer.alloc()"
  ...
  if (!authenticator) {
    if (uses_random_authenticator[args.code]) {
      ...
      //No.10 Line 744: changed "new Buffer" to "Buffer.alloc()"
...
Radius._encode_with_authenticator = function (
  args,
  packet,
  offset,
  authenticator
  ...
  if (add_message_authenticator) {
    //No.11 Line 649: changed "new Buffer" to "Buffer.alloc()"
  ...
  if (add_message_authenticator && !is_request_code[args.code]) {
    message_authenticator = this.calculate_message_authenticator(
      packet,
      args.secret
    );
  ...
  if (!uses_random_authenticator[args.code]) {
    this.calculate_packet_checksum(packet, args.secret).copy(
      packet,
      AUTH_START
    );
  }
...
Radius.calculate_message_authenticator = function (packet, secret) {
  ...
  hmac.update(packet);
  //No.12 Line 839: changed "new Buffer" to "Buffer.from()"
...
Radius.calculate_packet_checksum = function (packet, secret) {
  ...
  //No.13 Line 847: changed "new Buffer" to "Buffer.from()"
  ...
  if (typeof attributes == "object" && !Array.isArray(attributes)) {
    ...
    for (var name in attributes) {
      ...
      if (typeof val == "object") {
        throw new Error(
          "Cannot have nested attributes when using hash syntax. Use array syntax instead"
        );
      }
...
Radius.encode_attributes = function (packet, attributes, vendor) {
  ...
  for (var i = 0; i < attributes.length; i++) {
    ...
    if (!attr_info && !(attr[1] instanceof Buffer)) {
      throw new Error(
        "encode: invalid attributes - must give Buffer for " +
          "unknown attribute '" +
          attr[0] +
          "'"
      );
      ...
      if (attr_info[ATTR_MODIFIERS]["encrypt=1"]) {
        ...
        switch (attr_info[ATTR_TYPE]) {
          ...
          case "text":
            ...
            //No.14 Line 906: changed "new Buffer" to "Buffer.from()"
            out_value = Buffer.from(in_value + "", "utf8");
            break;
          case "ipaddr":
            //No.15 Line 911: changed "new Buffer" to "Buffer.from()"
          ...
          case "time":
          case "integer":
            //No.16 Line 921: changed "new Buffer" to "Buffer.alloc()"
          ...
          default:
            if (attr_info[ATTR_NAME] != "Vendor-Specific") {
              throw new Error(
                "encode: must provide Buffer for attribute '" +
                  attr_info[ATTR_NAME] +
                  "'"
              );
            }
        ...
        if (attr_info[ATTR_NAME] == "Vendor-Specific") {
          ...
          var length = this.encode_attributes(
            packet.slice(offset + 5),
            attr[2],
            vendor_id
          );
